...
%% PDE System Components
    ...
    lambda = E.Reaction.lambda * 96485; % Convert from eV to J/mol
    k0 = E.Reaction.k0;
    
    % Forward (oxidation) rate constant - Red → Ox + e-
    k_ox = k0 * exp(-((lambda - E.Reaction.n*C.F*eta)^2)/(4*lambda*C.R*C.T));
    
    % Backward (reduction) rate constant - Ox + e- → Red
    k_red = k0 * exp(-((lambda + E.Reaction.n*C.F*eta)^2)/(4*lambda*C.R*C.T));
    
    % Net flux - positive flux means oxidation of reduced species
    flux = k_ox*ul(1) - k_red*ul(2);
    
    % Boundary conditions remain the same
    pl = [flux; flux];  % Flux terms for both species
    ql = [-1; 1];       % Flux directions
    pr = [0; 0];        % No source terms
    qr = [1; 1];        % Zero flux far from electrode
end
